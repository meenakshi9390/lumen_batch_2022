...
import com.example.demo.services.MobilePlanServices;
...
public class MobilePlanController {
	private MobilePlanServices service;
    ...
    public ResponseEntity<MobilePlan> add(@RequestBody MobilePlan entity){
       ...
       URI location = ServletUriComponentsBuilder
    		              .fromCurrentRequest()
    		              .path("/{id}").buildAndExpand(entity.getPlanId())
    ...
    public ResponseEntity<String>remove (@PathVariable("id") int id){
        ...
        MobilePlan delElement =this.service.remove(id);
    ...
    public List<MobilePlan> getCostGrtThan(@PathVariable("cost") double cost){
        return this.service.findCostGrtThan(cost); 
    }
    @PatchMapping("/{planName}/{updatedValue}")
    public ResponseEntity<String> updatedValidity(@PathVariable("updatedValue")
